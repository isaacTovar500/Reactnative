...
class EmpleadoScreen extends Component {
  ...
  getCollection = (querySnapshot) => {
    ...
    querySnapshot.forEach((res) => {
      const { apellidoPaterno, apellidoMaterno, nombre, direccion, telefono  } = res.data();
      userArr.push({
        key: res.id,
        res,
        apellidoPaterno,
...
<ScrollView style={styles.container}>
      <Button
        onPress={() => this.props.navigation.navigate("AddEmpleadoScreen")}
      ...
      {this.state.userArr.map((item) => {
        return (
          <ListItem
            ...
            bottomDivider
            onPress={() => {
              this.props.navigation.navigate("EmpleadoDetailScreen", {
                userkey: item.key,
              });
            }}
          >
